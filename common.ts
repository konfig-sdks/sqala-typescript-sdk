...
import { readableStreamToString, SqalaError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new SqalaError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new SqalaError(e, e.response?.data)
                }
                if (innerError instanceof SqalaError) {
                    // Got "SqalaError" from the above try block
                    throw innerError;
                }
